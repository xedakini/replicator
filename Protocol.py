...
def connect(addr):
    ...
    if addr not in DNSCache:
        ...
        DNSCache[addr] = socket.getaddrinfo(addr[0], addr[1], socket.AF_UNSPEC,
                                            socket.SOCK_STREAM)
...
class HttpProtocol(Cache.File):
    ...
    def __init__(self, request):
        ...
        if stat:
            ...
            if self.partial():
                logging.info(
                    f'Requesting resume of partial file in cache: {size} bytes, {mtime}'
                )
                args[b'Range'] = b'bytes=%i-' % size
            else:
                logging.info(
                    f'Checking complete file in cache: {size} bytes, {mtime}')
                args[b'If-Modified-Since'] = mtime.encode()

        self.__socket = connect(request.addr)
        self.__sendbuf = b'\r\n'.join(
            [head] + list(map(b': '.join, iter(args.items()))) + [b'', b''])
        self.__recvbuf = b''
    ...
    def __parse_head(self, chunk):
        ...
        fields = line.split()
        assert len(fields) >= 3 and fields[0].startswith(
            b'HTTP/') and fields[1].isdigit(), 'invalid header line: %r' % line
    ...
    def recv(self, sock):
        ...
        if self.__status == 200:
                ...
                for timefmt in OPTS.timefmt:
                    try:
                        mtime = calendar.timegm(
                            time.strptime(
                                self.__args[b'Last-Modified'].decode(),
                                timefmt))
                    except ValueError:
                    ...
                    raise ValueError(
                        "time data '%s' does not match formats %s" %
                        (self.__args[b'Last-Modified'], b', '.join(
                            "'%s'" % timefmt for timefmt in OPTS.timefmt)))
            ...
            assert range.startswith(
                b'bytes '), 'invalid content-range: %s' % range
    ...
    def recvbuf(self):
        return b'\r\n'.join(
            [b'HTTP/1.1 %i %s' % (self.__status, self.__message)] +
            list(map(b': '.join, iter(self.__args.items()))) + [b'', b''])
...
class FtpProtocol(Cache.File):
    ...
    def recv(self, sock):
        ...
        while b'\n' in self.__recvbuf:
            ...
            if reply[:3].isdigit() and reply[3:4] != b'-':
                ...
                if self.__sendbuf:
                    logging.log(DEBUG2,
                                f'C: {self.__sendbuf.rstrip().decode()}')
    ...
    def __handle_mtime(self, code, line):
        ...
        assert code == 213, 'server sends %i; expected 213 (file status)' % code
        self.mtime = calendar.timegm(
            time.strptime(line.decode().rstrip(), '%Y%m%d%H%M%S'))
        logging.info(
            f'Modification time: {time.strftime(OPTS.timefmt[0], time.gmtime(self.mtime))}'
        )
        stat = self.partial()
        if stat:
