...
class DataResponse:
    ...
    def __init__(self, protocol, request):
        ...
        if self.__protocol.mtime >= 0:
            args[b'Last-Modified'] = time.strftime(
                OPTS.timefmt[0], time.gmtime(self.__protocol.mtime)).encode()
        if self.__pos == 0 and self.__end == self.__protocol.size:
            ...
            if self.__protocol.size >= 0:
                args[b'Content-Range'] = b'bytes %i-%i/%i' % (
                    self.__pos, self.__end - 1, self.__protocol.size)
            else:
                args[b'Content-Range'] = b'bytes %i-%i/*' % (self.__pos,
                                                             self.__end - 1)
        else:
    ...
    def recv(self, sock):
        ...
        if chunk:
            ...
            if self.__protocol.size >= 0:
                ...
                logging.info(
                    f'Connection closed at byte {self.__protocol.size}')
...
class DirectResponse:
    ...
    def __init__(self, status, request):
        ...
        for line in head.splitlines():
            lines.append(b'  %s' % line if len(line) <= 78 else b'  %s...' %
                         line[:75])
        if body:
