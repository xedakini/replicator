...
class File:
    ...
    def __init__(self, origpath):
        ...
        if OPTS.maxfilelen > -1:
            newpath = os.sep.join(
                item if len(item) <= OPTS.maxfilelen else
                item[:OPTS.maxfilelen - 34] + '..' +
                hashlib.md5(item[OPTS.maxfilelen - 34:]).hexdigest()
                for item in path.split(os.sep))
            if newpath != path:
                logging.info(
                    'Shortened path to %s characters',
                    '/'.join(str(len(w)) for w in newpath.split(os.sep)))
                path = newpath
        ...
        if path[0] == os.sep or path == '..' or path[:3] == '..' + os.sep:
            raise AssertionError(
                f'requested cache path outside of cache root: {origpath}')
